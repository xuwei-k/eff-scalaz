...
type Stack = Fx.fx4[KVStore, Throwable \/ ?, State[Map[String, Any], ?], Writer[String, ?]]

val (result, logs) =
  runKVStore(program[Stack]).runDisjunction.evalState(Map.empty[String, Any]).runWriter.run
...
val (result, logs) =
  program[Stack].runStore.runDisjunction.evalState(Map.empty[String, Any]).runWriter.run
