...
runT1(runT4(runT2(runT3(Eff.send[T3, S, Int](T3(1))))))
...
A common thing to do is to translate "high-level" effects (a webservice DSL for example) into low-level ones (`Future`, `Eval`, `\/`, etc...).

For example you might have this stack:
  ```
type S = Fx.fx3[Authenticated, Future, ThrowableOr]
```

And you want to write an interpreter which will translate authentication actions into `Future` and `\/`:${snippet{
...
type AuthErrorDisjunction[A] = AuthError \/ A
type _error[R] = AuthErrorDisjunction |= R
     ...
     def apply[X](ax: Authenticated[X]): Eff[U, X] =
       ax match {
         case Authenticate(token) =>
           ...
           // send the \/ value in the stack U
           collapse
       }
    })

// call to a service to authenticate tokens
def authenticate(token: String): Future[AuthError \/ AccessRights] = ???

type S = Fx.fx3[Authenticated, AuthError \/ ?, Future]
def auth: Eff[S, Int] = ???
...
Furthermore, `authenticate` returns an `AuthError \/ ?` value. We can "collapse" it into `U` because `AuthError \/ ?`
is an effect of `U` as evidenced by `disjunction`.
